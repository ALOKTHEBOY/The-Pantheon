...
export function ProductDetails(id) {
  // Convert URL string ID to a number and find the product
  const product = products.find(p => p.id === parseInt(id));

  // Fallback if someone types an invalid ID in the URL
  if (!product) {
    return `
  ...
  return `
    ...
    <div class="grid grid-cols-2" style="gap: var(--spacing-lg); background: var(--color-surface); padding: var(--spacing-lg); border-radius: var(--radius-md); border: 1px solid var(--color-border);">
      <div>
      ...
      <div style="display: flex; flex-direction: column; justify-content: center;">
        ...
        <p style="margin-bottom: var(--spacing-lg); line-height: 1.6; color: var(--color-text-muted);">
          This is a detailed description of the ${product.name}. Built with premium materials to ensure the highest quality and durability for our customers. Experience the perfect blend of functionality and style.
        </p>
