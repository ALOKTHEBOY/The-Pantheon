    ...
    <div style="max-width: 1000px; margin: 2rem auto; padding: 0 1rem;">
      ...
      <div style="padding: var(--spacing-lg); background: var(--color-surface); border-radius: var(--radius-md); border: 1px solid var(--color-border);">
        ...
        <div style="display: flex; gap: 10px; margin-bottom: var(--spacing-md);">
          <input type="text" id="dashboard-search" placeholder="Search products..." style="flex: 1; padding: 8px; border-radius: 4px; border: 1px solid var(--color-border); background: var(--color-background); color: var(--color-text-main);">
...
export async function initDashboardProductList() {
  ...
  function renderList() {
    ...
    listContainer.innerHTML = filtered.map(product => `
      <div class="dashboard-product-row" data-id="${product.id}" style="display: flex; justify-content: space-between; align-items: center; padding: 12px; border: 1px solid var(--color-border); border-radius: 6px; background: var(--color-background);">
        <div style="display: flex; align-items: center; gap: 12px;">
          ...
          <div>
            <div style="font-weight: bold; color: var(--color-text-main); font-size: 0.95rem;">${product.name}</div>
        ...
        <div style="display: flex; gap: 10px;">
          <!-- NEW: Dedicated Edit Button Route -->
